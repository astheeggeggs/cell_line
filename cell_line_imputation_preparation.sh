...
perl HRC-1000G-check-bim.pl -b data/celldataB37_dp_test.bim -f data/celldataB37_dp_test.frq -t 0.2 -r ${RefPanel} -h
...
# Build 38
cd ..
perl CreateTOPMed.pl -i data/TOPMed_variants/bravo-dbsnp-all.GRCh38.vcf.gz -o data/for_imputation/PASS.topmed.GRCh38.tab.gz
...
# -t default is 0.2 but I set it to 1 so that no variants are removed because have have so few samples
# Submit a job (RAM ~60G)
perl HRC-1000G-check-bim.pl -b data/celldataB38_test.bim -f data/celldataB38_test.frq -t 0.2 -r ${RefPanel} -h
...
for chr in {1..23}
do
    awk '{if($0 !~ /^#/) print "chr"$0; else print $0}' celldataB38_test-updated-chr${chr}.vcf > tmp && mv tmp celldataB38_test-updated-chr${chr}.vcf
